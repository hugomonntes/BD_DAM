...
--28
SELECT APELLIDO, IDDEPART FROM empleados WHERE IDDEPART = (SELECT IDDEPART FROM depart WHERE LOC LIKE 'Madrid') OR IDDEPART = (SELECT IDDEPART FROM depart WHERE LOC = 'Barcelona') ORDER BY IDDEPART;
--29
SELECT IDDEPART, NOMBRE FROM depart GROUP BY IDDEPART HAVING IDDEPART NOT IN (SELECT IDDEPART FROM depart WHERE COUNT(*) != 0);
--30
-- 30. Obtener el apellido y oficio de los empleados que no son de Sevilla ni de Madrid y de
-- los que se desconoce su comisión.
SELECT APELLIDO, OFICIO FROM empleados GROUP BY IDDEPART HAVING IDDEPART IN (SELECT IDDEPART FROM depart WHERE LOC != 'SEVILLA' AND LOC != 'MADRID' AND COMISION IS NULL);